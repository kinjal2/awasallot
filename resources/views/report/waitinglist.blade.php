    ...
    $(document).ready(function() {
        ...
        var table = $('#waitinglist').DataTable({
            ...
            createdRow: function(row, data, dataIndex) {
                ...
                // console.log(data.date_of_retirement, retired, new Date(data.date_of_retirement), new Date());
                // Condition 2: remarks exist (not "Add Remarks")
                // ✅ Fix retirement comparison
                var retired = false;
                if (data.date_of_retirement) {
                    let retirementDate = parseDMY(data.date_of_retirement);
                    retired = retirementDate && retirementDate < new Date();
                    // console.log(
                    //     "Retirement:", data.date_of_retirement,
                ...
                if (retired) {
                    $(row).children('td').css('background-color', '#0B5711');
                    if(!hasRemarks)
                    {
                        var remarksCell = $(row).find('td').eq(9);
                        remarksCell.html(''); // blank remarks
                    }
                }
