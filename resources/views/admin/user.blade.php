...
<script type="text/javascript">
    var table = $('#userlist').DataTable({
        ...
        columns: [{
            ...
            {
                data: 'date_of_birth_link',
                name: 'date_of_birth',
                searchable: true
            },
            {
                data: 'designation_link',
                name: 'designation',
                searchable: true
            },
            {
    ...
    */
    ...
    // Create footer search inputs ONLY for searchable columns
    $('#userlist tfoot th').each(function(index) {
        var column = table.column(index);

        if (column.settings()[0].aoColumns[index].searchable) {
            $(this).html(
                '<input type="text" placeholder="Search" style="width:100%" />'
            );
        } else {
            $(this).html(''); // No search box
        }
    });
    table.columns().every(function() {
        var that = this;
