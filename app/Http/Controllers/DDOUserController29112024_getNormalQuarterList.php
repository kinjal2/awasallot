...
class DDOUserController extends Controller
{
    ...
    public function geteditquarter_a (Request $request, $requestid, $rivision_id)
    {
       ...
       // DB::enableQueryLog();
        ...
        ->whereNotIn('document_type',  Filelist::WHERE('uid', $quarterrequest['uid'])
            ...
            ->pluck('document_id','is_file_ddo_verified'))
    ...
    ->where('is_accepted', '=', 1)
    ->where('is_ddo_varified','=',0);  //filter by is_accepted
...
$union = DB::table('master.t_quarter_request_b AS b')
    ...
    ->join('userschema.users as u', 'u.id', '=', 'b.uid')
    ->join('master.m_ddo as d', 'd.ddo_code', '=', 'u.ddo_code') // Join the ddo table
    ->where('d.ddo_code', $ddoCode) // Filter by ddo_code
    ->where('d.cardex_no',$cardexNo)
    ->where('officecode', $officeCode) // Filter by office_code
    ->where('is_accepted', '=', 1)  //filter by is_accepted
    ->where('is_ddo_varified','=',0)
    ->union($first)
    ->union($second);
