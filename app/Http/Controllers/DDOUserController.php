...
class DDOUserController extends Controller
{
    ...
    public function getNormalquarterList_old(Request $request)
    {
        ...
        'address','gpfnumber','is_accepted','is_allotted','is_varified','email','is_priority','u.ddo_code',				  	 
    'u.cardex_no'])
    ...
    public function getNormalquarterList(request $request)
    {
        ...
        $officeCode = session('officecode'); // Get the office code from session
            $first = DB::table('master.t_quarter_request_a AS a')
                ->select(['request_date', DB::raw("'a'::text as type"), DB::raw("'New'::text as requesttype"), 'requestid', 'quartertype',
                'inward_no', 'inward_date','u.name', 'u.designation', 'office','rivision_id','a.remarks', 'contact_no', 'address',      'gpfnumber', 'is_accepted', 'is_allotted','is_varified',  'email', 'is_priority', 'a.ddo_code',  'a.cardex_no' ])
                ->join('userschema.users as u', 'u.id', '=', 'a.uid')
...
// $document_list = Documenttype::where('performa', 'LIKE', '%'. $type.'%')->whereNotIn('document_type',  Filelist::WHERE('uid',Session::get('Uid'))->WHERE('request_id',$request_id)->WHERE('performa',$type)->pluck('document_id'))
        // ->pluck('document_name','document_type');